void main_setup() { //Case 3: Cone flip JOHNS; required extensions in defines.hpp: FP16S, EQUILIBRIUM_BOUNDARIES, SUBGRID, INTERACTIVE_GRAPHICS or GRAPHICS
	...
	const float lbm_length = 0.23f*(float)lbm_N.y;
	...
	Mesh* mesh = read_stl(get_exe_path()+"/stl/SimpleConeFlip.stl", lbm.size(), lbm.center(), rotation, lbm_length);
	mesh->translate(float3(0.0f, mesh->pmin.y-3.6f*lbm_length, 0.0f));
	lbm.voxelize_mesh_on_device(mesh);
	Mesh* wmesh = read_stl(get_exe_path()+"/stl/Wall.stl", lbm.size(), lbm.center(), rotation, lbm_length);
	wmesh->translate(float3(0.0f, wmesh->pmin.y-3.6f*lbm_length, 0.0f));
